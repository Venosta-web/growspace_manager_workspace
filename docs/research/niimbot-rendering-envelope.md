...
- The Home Assistant service UI advertises canvas widths and heights from 10 through 1600 pixels and whole-label rotations of 0, 90, 180, or 270 degrees ([service-field metadata](https://github.com/eigger/hass-niimbot/blob/f2bed90599dfe4459a80079fd3430d93d26caf40/custom_components/niimbot/services.yaml#L22-L45)). These UI bounds are not a promise that a chosen printer can physically print every selectable size.
...
- The width/height selector bounds are not an enforced service schema: the registered handler receives raw call data, while `imagespec` only rejects non-positive canvas dimensions and disallowed rotations ([service registration](https://github.com/eigger/hass-niimbot/blob/f2bed90599dfe4459a80079fd3430d93d26caf40/custom_components/niimbot/__init__.py#L342-L347), [renderer validation](https://github.com/eigger/imagespec/blob/34c8bf2100fc3c6b04467e986491e679cddfdc81/src/imagespec/core.py#L24-L42)). Canonical validation cannot rely on the UI's 10–1600 range.
...
- Niimbot's adapter resolves font basenames first from `custom_components/niimbot/fonts/`, then from Home Assistant's `www/fonts/`, and configures its shipped `ppb.ttf` as the default ([font resolver](https://github.com/eigger/hass-niimbot/blob/f2bed90599dfe4459a80079fd3430d93d26caf40/custom_components/niimbot/render.py#L9-L42)). `imagespec` tries the injected resolver, a bundled font with the requested basename, and then a bundled font matching the configured default name before raising `RenderError` ([font fallback](https://github.com/eigger/imagespec/blob/34c8bf2100fc3c6b04467e986491e679cddfdc81/src/imagespec/context.py#L70-L99)). For an explicitly named missing font, it does not retry the integration resolver with `ppb.ttf`; the payload can therefore fail even though the integration's default font exists.
